...
def td3(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=None, 
    ...
    # Experience buffer
    replay_buffer = {i: ReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size) for i in es_idxs}
    ...
    g = tf.Graph()
    with g.as_default():
        __, _get_solo_action = load_policy_and_env(osp.join(DEFAULT_DATA_DIR, load_1vs0), num, sess=None)
    ...
    def test_agent():
        ...
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                act_1 = get_solo_action(o[0])
                a = act_1 + get_action(np.array(o[1:]), 0, num_players - 1)
                ...
                ep_len += 1
            success_rate += (ep_len <= max_ep_len and test_env.timestep.reward[1] > 0) / num_test_episodes
    ...
    for t in range(total_steps):
        ...
        act_1 = get_solo_action(o[0])
        if t > start_steps:
            a = act_1 + get_action(np.array(o[1:]), act_noise, num_players - 1)
        else:
            a = act_1 + [env.action_space.sample() for _ in range(1, num_players)]
        ...
        if t >= update_after and t % update_every == 0:
            for j in range(update_every):
                batch_dicts = [replay_buffer[j].sample_batch(batch_size // len(es_idxs)) for j in es_idxs]
                batch = {key: np.concatenate([batch_dicts[i][key] for i in range(len(es_idxs))], axis=0) for key in batch_dicts[0].keys()}
...
if __name__ == '__main__':
    ...
    logger_kwargs = setup_logger_kwargs(f'td3_soccer_1vs1_{es_tag}{args.reward}_{args.control_timestep}', data_dir=osp.join(DEFAULT_DATA_DIR, "TD3/1vs1"), datestamp=True)
    if args.gpu > 0:
    ...
    td3(lambda : dm_soccer2gym.make('1vs1', task_kwargs={"rew_type": args.reward, "time_limit": args.time_limit, "disable_jump": True, 
        "dist_thresh": 0.03, 'control_timestep': args.control_timestep}), 
        test_env_fn=lambda : dm_soccer2gym.make('1vs1', task_kwargs={"rew_type": "simple_v2", "time_limit": args.time_limit, "disable_jump": True, 
        ...
        load_1vs0="TD3/1vs0/2020-09-12_23-40-25_td3_soccer_1vs0_dense_0.05",
        num=8299999, use_es=args.use_es)
